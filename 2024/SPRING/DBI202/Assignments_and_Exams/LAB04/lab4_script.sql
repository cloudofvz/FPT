...
WITH
    ...
    (
        SELECT
            e.empSSN, e.empName, SUM(w.workHours) as total_ews
        FROM tblEmployee e
            INNER JOIN tblWorksOn w ON e.empSSN = w.empSSN
            INNER JOIN tblDepartment d ON e.depNum = d.depNum
        GROUP BY e.empSSN,
...
FROM pc
WHERE total_pro = 1;
--Lần đầu tham gia dự án nên đếm số dự án tham gia là 1
...
SELECT proNum,
    ...
    total_emp
FROM p_ec
WHERE total_emp = (SELECT MIN(total_emp)
FROM p_ec);
...
SELECT proNum,
    ...
    total_emp
FROM p_ec
WHERE total_emp = (SELECT MAX(total_emp)
FROM p_ec);
...
SELECT *
FROM pws
WHERE total_ws = (SELECT MIN(total_ws)
FROM pws);
...
SELECT *
FROM pws
WHERE total_ws = (SELECT MAX(total_ws)
FROM pws);
...
SELECT *
FROM lc_eachdep
WHERE total_dep = (SELECT MAX(total_dep)
FROM lc_eachdep);
...
SELECT *
FROM lc_eachdep
WHERE total_dep = (SELECT MIN(total_dep)
FROM lc_eachdep);
...
SELECT *
FROM dpc_eache
WHERE total_dp = (SELECT MAX(total_dp)
FROM dpc_eache);
...
SELECT *
FROM dpc_eache
WHERE total_dp = (SELECT MIN(total_dp)
FROM dpc_eache);
...
SELECT *
FROM totalp_eachd
WHERE total_p = (SELECT MIN(total_p)
FROM totalp_eachd);
...
SELECT *
FROM totalp_eachd
WHERE total_p = (SELECT MAX(total_p)
FROM totalp_eachd);
...
FROM tblEmployee e
    INNER JOIN tblDepartment d ON e.depNum = d.depNum
WHERE d.depName = N'Phòng Nghiên cứu và phát triển' AND e.empSSN NOT IN (SELECT empSSN
    FROM tblDependent);
...
WITH
    ...
    AS
    (
        SELECT e.empSSN, e.empName, COUNT(dp.depName) as total_dps
        FROM tblEmployee e
            INNER JOIN tblDependent dp ON e.empSSN = dp.empSSN
        GROUP BY e.empSSN,e.empName
    )
SELECT dps.empSSN, dps.empName, dps.total_dps, SUM(w.workHours) as total_ws
FROM dps
    INNER JOIN tblWorksOn w ON dps.empSSN = w.empSSN
WHERE total_dps > 3
GROUP BY dps.empSSN,dps.empName,dps.total_dps
...
FROM tblEmployee e1
    ...
    LEFT JOIN tblWorksOn w ON e1.empSSN = w.empSSN
